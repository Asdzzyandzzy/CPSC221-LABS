...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
     ...
     */
     ...
     // 1. We are about to add a new element, so increment elems:
     elems++;

     // 2. Check load factor AFTER incrementing, resize if necessary
     if ((float)elems / size >= 0.7) {
         resizeTable();
     }
 
     // 3. Find the index with the hash function
     size_t idx = hashes::hash(key, size);
 
     // 4. Linear probe: move forward until we find an empty slot (NULL)
     while (table[idx] != NULL) {
         idx = (idx + 1) % size;
     }
 
     // 5. Create a new pair for (key, value)
